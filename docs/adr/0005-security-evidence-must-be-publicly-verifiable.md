...
- **Its honest limit: nothing here runs the tests.** The README asserts 209 passing tests, `eslint` clean and `next build` green, and no workflow verifies any of it — those are checkable only by cloning and running, which is more than reading and impossible for a reviewer who only reads. By this ADR's own standard that is the largest remaining gap, and it is a CI decision rather than a scanning one.
